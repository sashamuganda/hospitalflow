...
## 2026-05-31 - Hoisting Transformations in Closures
**Learning:** Performing expensive operations like `String.toLowerCase()` inside a `where` or `map` closure results in $O(N)$ repeated operations that could be computed once.
**Action:** Hoist any constant transformations (like lowercasing a search query or a filter key) outside of the collection closure to optimize inner-loop performance.
