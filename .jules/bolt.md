...
## 2026-06-01 - Redundant String Transformations in Filter Loops
**Learning:** Performing string transformations like `toLowerCase()` inside a `.where()` closure on the search query itself leads to $O(N)$ redundant operations (where $N$ is the number of items in the list).
**Action:** Hoist the query transformation outside of the filtering loop to a local variable at the beginning of the `build` method.
