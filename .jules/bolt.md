...
**Learning:** List sorting (O(N log N)) is significantly more expensive than simple filtering. When a sorting getter is accessed multiple times in a `build` method (e.g., for `itemCount` and then for each item in `itemBuilder`), the performance hit is substantial, especially for larger lists.
**Action:** Prioritize caching sorted lists over simple filtered ones in build methods to maximize performance gains.
