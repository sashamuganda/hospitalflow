...
GoRouter createRouter(AppState appState) {
  return GoRouter(
    ...
    redirect: (context, state) {
      ...
      // Security: RBAC check for administrative routes
      final isAdminRoute =
    ...
    routes: [
      ...
      ShellRoute(
        ...
        routes: [
          ...
          GoRoute(
              path: '/appointments', builder: (_, __) => const StaffAppointmentsHome()),
          GoRoute(path: '/ward', builder: (_, __) => const WardOverviewScreen()),
          GoRoute(
              path: '/telemedicine', builder: (_, __) => const TeleHomeStaffScreen()),
          ...
          GoRoute(
              path: '/analytics', builder: (_, __) => const AnalyticsHomeScreen()),
      ...
      GoRoute(
          path: '/queue/wait-times', builder: (_, __) => const WaitingTimesScreen()),
      GoRoute(
      ...
      GoRoute(
        ...
        builder: (_, state) =>
            ClinicalNoteEditor(patientId: state.pathParameters['patientId'] ?? ''),
      ),
      GoRoute(
        path: '/emr/prescription/:patientId',
        builder: (_, state) =>
            PrescriptionWriterScreen(patientId: state.pathParameters['patientId'] ?? ''),
      ),
      GoRoute(
        path: '/emr/vitals/:patientId',
        builder: (_, state) =>
            VitalsEntryStaff(patientId: state.pathParameters['patientId'] ?? ''),
      ),
      GoRoute(
      ...
      GoRoute(
          path: '/emr/lab-order', builder: (_, __) => const LabOrderScreen(patientId: '')),
    ],
  );
}
