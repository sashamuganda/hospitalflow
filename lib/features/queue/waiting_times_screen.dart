...
class WaitingTimesScreen extends StatelessWidget {
              ...
              Expanded(
                child: ListView.separated(
                  ...
                  itemBuilder: (context, i) =>
                      _DeptWaitCard(stats: departments[i]),
                ),
              ),
            ],
          ),
  ...
  List<_DeptStats> _deptStats() {
    ...
    return byDept.entries.map((e) {
      final waiting =
          e.value.where((p) => p.status == QueueStatus.waiting).length;
      final avgWait = e.value.isEmpty
          ? 0
          : e.value.map((p) => p.waitMinutes).reduce((a, b) => a + b) ~/
              e.value.length;
      final hasImmediate =
          e.value.any((p) => p.triageLevel == TriageLevel.immediate);
      return _DeptStats(
          name: e.key,
          patientCount: e.value.length,
            ...
            Expanded(
              child: Column(
                  ...
                  children: [
                    ...
                    Text(
                        '${stats.patientCount} patients · ${stats.waitingCount} waiting',
