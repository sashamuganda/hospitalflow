...
class _NotificationsScreenState extends State<NotificationsScreen> {
  ...
  Widget build(BuildContext context) {
    // ⚡ Bolt: Consolidate unread count and filtered list generation into a single O(N) pass.
    // Caching results in local variables eliminates redundant O(N) traversals and
    // prevents the O(N*M) complexity overhead of repeatedly calling getters within the widget tree.
    int unreadCount = 0;
    final filtered = <StaffNotification>[];
    final filterLower = _filter.toLowerCase();

    for (final n in mockNotifications) {
      if (!n.isRead) unreadCount++;

      if (_filter == 'All' || n.type.toLowerCase() == filterLower) {
        filtered.add(n);
      }
    }
