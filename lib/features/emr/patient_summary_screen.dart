...
class _PatientSummaryScreenState extends State<PatientSummaryScreen>
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                    '${p.age}y · ${p.gender} · DOB: ${_fmtDate(p.dateOfBirth)}',
                ...
                Row(children: [
                  ...
                  if (p.insuranceProvider != null)
                    RoleBadge(
                        label: p.insuranceProvider!, color: AppColors.success),
                ]),
              ],
            ),
          ),
        ...
        children: [
          ...
          p.activeConditions.isEmpty
              ...
              : Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: p.activeConditions
                      .map((c) =>
                          StatusBadge(label: c, color: AppColors.secondary))
                      .toList()),
          ...
          p.allergies.isEmpty
              ...
              : Wrap(
                  ...
                  children: p.allergies
                      .map((a) =>
                          StatusBadge(label: '⚠ $a', color: AppColors.error))
                      .toList()),
          const SizedBox(height: 20),
  ...
  Widget _buildVitalsTab(BuildContext context) {
    final vitalsList =
        mockVitals.where((v) => v.patientId == widget.patientId).toList();
  ...
  Widget _buildNotesTab(BuildContext context) {
    final notes = mockClinicalNotes
        .where((n) => n.patientId == widget.patientId)
        .toList();
    return notes.isEmpty
        ? EmptyState(
            ...
            onAction: () =>
                context.push('/emr/note-editor/${widget.patientId}'))
        : ListView.separated(
  ...
  Widget _buildLabsTab(BuildContext context) {
    final orders =
        mockLabOrders.where((l) => l.patientId == widget.patientId).toList();
                    ...
                    children: [
                      ...
                      Wrap(
                          ...
                          children: o.tests
                              ...
                              .toList()),
                      const SizedBox(height: 8),
                      Text('Ordered by: ${o.orderedByName}',
                          style: Theme.of(context).textTheme.bodySmall),
                      Text('Specimen: ${o.specimenType ?? "—"}',
                          style: Theme.of(context).textTheme.bodySmall),
                    ]),
              );
            });
  ...
  Widget _emptyChip(String text) => Container(
      ...
      child: Text(text,
          style: const TextStyle(
              fontSize: 13, color: AppColors.textMuted, fontFamily: 'Inter')));
...
class _VitalsCard extends StatelessWidget {
    ...
    return GlassCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text('Vitals — ${_fmt(vitals.timestamp)}',
              style: Theme.of(context).textTheme.titleSmall),
          const Spacer(),
          Text(vitals.recordedBy,
              style: const TextStyle(
                  fontSize: 10,
        ...
        Wrap(spacing: 16, runSpacing: 8, children: [
          ...
          if (vitals.heartRate != null)
            _vItem('HR', '${vitals.heartRate!.toInt()}', 'bpm'),
          if (vitals.spO2 != null)
            _vItem('SpO₂', '${vitals.spO2!.toInt()}', '%'),
          if (vitals.temperature != null)
            _vItem('Temp', '${vitals.temperature}', '°C'),
  ...
  Widget _vItem(String l, String v, String u) =>
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(l,
            style: const TextStyle(
                fontSize: 11, color: AppColors.textMuted, fontFamily: 'Inter')),
  ...
  Widget build(BuildContext context) {
    return GlassCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(
              child: Text(note.doctorName,
                  style: Theme.of(context).textTheme.titleSmall)),
          StatusBadge(
              label: note.isSigned ? 'Signed' : 'Draft',
              color: note.isSigned ? AppColors.success : AppColors.warning,
              fontSize: 10),
        ]),
